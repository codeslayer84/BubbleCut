...
interface Props {
  ...
  width: number;
  height: number;
  /** Drawn dimmer when the track is not selected. */
  selected: boolean;
}
 ...
 */
export function Waveform({ path, inPoint, outPoint, duration, width, height, selected }: Props) {
  ...
  useEffect(() => {
    ...
    for (let x = 0; x < env.length; x++) {
      ...
      const half = Math.max(0.5, env[x] * (mid - 1));
      g.fillRect(x, mid - half, 1, half * 2);
    }
  }, [peaks, inPoint, outPoint, duration, width, height, selected]);
