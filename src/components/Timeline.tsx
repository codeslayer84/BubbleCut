...
export function Timeline() {
  ...
  // The audio lane mirrors the clips: it is their own sound, so it cannot be
  // moved or trimmed independently — dragging it would only mean dragging the
  // clip, which the clip row already does.
  const AUDIO_ROW_H = 44;
  const anyAudio = clips.some((c) => !c.fill && (media[c.mediaPath]?.hasAudio ?? false));
  const audioLaneH = anyAudio ? AUDIO_ROW_H : 0;
  ...
  const autoHeight = 152 + (cards.length ? laneHeight + 8 : 0) + (anyAudio ? audioLaneH + 8 : 0);
        ...
        <div className="track" style={{ width: Math.max(total * pxPerSec + 40, 100) }}>
          ...
          {anyAudio && (
            <div className="audio-lane" style={{ height: audioLaneH }}>
              {clips.map((c) => {
                const m = media[c.mediaPath];
                if (c.fill || !m?.hasAudio) return null;
                const left = clipStart(clips, c.id) * pxPerSec;
                const w = Math.max(clipLength(c) * pxPerSec, 2);
                const sel = c.id === selectedId;
                const gain = c.audioGain ?? 1;
                const fadeIn = c.audioFadeIn ?? 0;
                const fadeOut = c.audioFadeOut ?? 0;
                return (
                  <div
                    key={c.id}
                    className={"tl-audio" + (sel ? " selected" : "") + (gain === 0 ? " muted" : "")}
                    style={{ left, width: w, height: AUDIO_ROW_H }}
                    title={`${m.name} — ${gain === 0 ? "muted" : `${Math.round(20 * Math.log10(gain || 1e-6))} dB`}`}
                    onPointerDown={() => selectClip(c.id)}
                  >
                    <Waveform
                      path={c.mediaPath}
                      inPoint={c.inPoint}
                    ...
                    />
                    {fadeIn > 0 && (
                      <div className="tl-audio-fade in" style={{ width: Math.min(fadeIn * pxPerSec, w / 2) }} />
                    )}
                    {fadeOut > 0 && (
                      <div className="tl-audio-fade out" style={{ width: Math.min(fadeOut * pxPerSec, w / 2) }} />
                    )}
                    {gain === 0 && <div className="tl-audio-muted">muted</div>}
                  </div>
                );
              })}
            </div>
