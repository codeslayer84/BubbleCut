...
Iridescent soap bubbles with a play badge, generated by `icon/make_icon.py`,
which draws it and writes raw RGBA for ffmpeg to encode — no image library is
installed on this machine. The film is drawn denser than a real bubble on
purpose: a physically faint one turns to mud at icon sizes. Regenerate
and re-cut the whole set with:

```bash
python3 icon/make_icon.py 1024 icon/icon.raw
ffmpeg -y -f rawvideo -pix_fmt rgba -s 1024x1024 -i icon/icon.raw icon/icon.png
cp icon/icon.png public/app-icon.png
